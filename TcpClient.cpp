...
bool TcpClient::Send(const std::string &text) {
	...
	{
		size_t toSendSize = dataLen - dataSend <= mReadBlockSize ? dataLen - dataSend : mReadBlockSize;
		int sendCnt = send(mSocketFd, dataPtr + dataSend, toSendSize, 0);
		if (sendCnt < 0) {
			int lastError = GetErrorNo();
			//TODO 判断是否需要断开连接
			DisConnect();
			return false;
		}
...
bool TcpClient::Read(std::string &text) {
	...
	int len = recv(mSocketFd, (char *)text.c_str(), mReadBlockSize, 0);
	if (len < 0) {
		//TODO 接受数据错误处理
		return false;
	}
...
bool TcpClient::ReadUtil(std::string & text, const std::string & util)
{
	...
	while (Read(tmpBuffer)) {
		...
		if ((pos = mReadBuffer.find(util)) != std::string::npos) {
			...
			memcpy((void*)text.c_str(), mReadBuffer.c_str(), text.size());
...
bool TcpClient::ReadAll(std::string &text, size_t len) {
	...
	char* dataPtr = const_cast<char *>(text.c_str());
